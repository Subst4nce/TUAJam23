...
public class PlayerGrabHandler : MonoBehaviour
{
    ...
    [SerializeField]
    private PlayerInputHandlerPlatformer playerInputManager;
    [SerializeField]
    ...
    void Start()
    {
        ...
        playerInputManager.input_interact.Onreleased.Subscribe(eventHandler, ReleaseObject);
    }
    ...
    private void Update()
    {
        ...
        viewDirLerped = viewDirLerped == Vector3.zero ? viewDir.forward.normalized : viewDirLerped;

        //var pos = viewDir.position + (Vector3.forward * distance);
        //var pos = viewDir.position + (viewDir.forward * distance);
        var pos = viewDir.position + (viewDirLerped * distance) + offset;
